...
Projekt: Mistrz Gry
## 1. Cel Sprintu
Głównym celem sprintu było przeprowadzenie pełnej, kontrolowanej rozgrywki z wykorzystaniem modelu AI w środowisku naszego autorskiego interfejsu. Skupiliśmy się na trzech aspektach:
...
Aplikacja przeszła chrzest bojowy. W trakcie wielogodzinnej sesji zidentyfikowano szereg błędów (_edge cases_), które wymagały natychmiastowej interwencji.
...
- **Ekonomii:** W _Rundzie 1_ AI podjęło decyzję o odpuszczeniu niepewnego konfliktu militarnego na rzecz budowy ekonomi, co zapewniło mu stabilność w środkowej części gry.
- **Atut Lidera:** Model bezbłędnie interpretował pasywne zdolności Duke'a Leto. W _Rundzie 2_ wykorzystał zniżkę Landsraadu, by zdobyć Mentata (dodatkowego agenta) znacznie taniej niż przeciwnicy.
- **Deck-Building:** Ruch na pole _Selective Breeding_ w celu usunięcia słabej karty startowej i dobrania nowych był podręcznikowym przykładem zaawansowanej gry.
...
W toku analizy logów zidentyfikowaliśmy zjawisko spadku jakości decyzji w czasie. Wynika ono z faktu, że AI otrzymywało w prompcie historię ruchów **tylko z bieżącej rundy**. Model "widział" planszę, ale nie pamiętał swoich długofalowych planów z przeszłości. Prowadziło to do następujących błędów strategicznych:
1. Syndrom "Zbieracza" (Błąd Deck-buildingu):
	AI nadmiernie kupowało tandetne karty, konkretnie *Arrakis Liaison*, zamiast oszczędzać na karty otwierające więcej drzwi. W efekcie, w decydujących momentach talia była rozwodniona słabszymi kartami.
2. Błędna Inwestycja w "Late Game":
    W przedostatniej rundzie AI wydało cenne zasoby na miejsce w High Council. Jest to inwestycja, która zwraca się po wielu rundach. Umożliwia kupowanie droższych kart, do wykorzystania dopiero po przetasowaniu kart. Na tym etapie należało skupić się na natychmiastowych punktach.
...
4. Amnezja Zasobowa (Intrygi):
    Najpoważniejszy błąd techniczny polegał na zapomnieniu o kartach intryg. Są to karty modyfikatory dające przewagę w rozgrywce. Nie powinno się ich jednak gromadzić, gdyż daje to możliwość reszcie graczy podebraniu jednej z nich. Raz jednak Peter zagrał intrygę co wyklucza opcję, że nie pamiętało nich całkiem.
